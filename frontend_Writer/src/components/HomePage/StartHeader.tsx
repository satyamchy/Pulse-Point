    ...
    return (
        <div className='bg-[#1B1F3B] text-[#F8F9FA] flex justify-between items-center py-4 px-6 shadow-lg shadow-[#D72638] mb-[4px]'>
            ...
            <nav className="hidden md:flex space-x-10 text-lg">
                ...
                {/* <NavLink className='hover:text-[#E63946] transition' to="/">My Articles</NavLink> */}
                <div className='hover:text-[#E63946] transition' onFocus={()=>{setAccount(true)}}>Login</div>
                <div className="hover:text-[#E63946] transition" onFocus={()=>{setAccount(!account)}}>Sign up</div>
            ...
            {account && (
                <div className='absolute top-16 right-2 items-start bg-[#313a79] text-white m-2 p-4 rounded-lg space-y-2 hidden md:block '>
                    <NavLink className="block hover:text-[#E63946] transition" to="/admin-login">Admin</NavLink>
                    <NavLink className='block hover:text-[#E63946] transition' to="/login">Author</NavLink>
                </div>
            )}
            ...
            {isOpen && (<div className='absolute top-16 right-0 items-start w-1/3 mid:1/3 bg-[#1B1F3B] text-white p-4 rounded-lg space-y-4 shadow-lg shadow-[#E63946] md:hidden'>
                ...
                <NavLink className='block' onClick={() => setIsOpen(false)} to="/logout">Sign out</NavLink> */}
                 <div className='hover:text-[#E63946] transition' onFocus={()=>{setAccount(!account)}}>Login</div>
                 <div className="hover:text-[#E63946] transition" onFocus={()=>{setAccount(!account)}}>Sign up</div>
            </div>
            )}
